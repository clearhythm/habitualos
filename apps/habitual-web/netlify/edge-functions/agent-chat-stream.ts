...
// Signal types that end the conversation
...
function hasSignal(text: string): boolean {
  return SIGNAL_PATTERNS.some((pattern) => pattern.test(text.trim()));
}
...
function parseSignal(
  ...
  for (const [index, pattern] of SIGNAL_PATTERNS.entries()) {
    if (pattern.test(trimmed)) {
      const types = ["GENERATE_ACTIONS", "GENERATE_ASSET", "STORE_MEASUREMENT"];
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const initResponse = await fetch(`${baseUrl}/api/agent-chat-init`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ userId, agentId, actionContext, reviewContext }),
  });
  ...
  const { systemMessages, tools, agent } = initData;
      ...
      try {
        ...
        while (continueLoop && loopCount < maxLoops) {
          ...
          for (const block of finalMessage.content) {
            ...
            if (block.type === "text") {
              // Full text already streamed, just track it
            }
          }
          ...
          if (toolUseBlock && finalMessage.stop_reason === "tool_use") {
            ...
            const signal = parseSignal(fullText);
            send({
              type: "done",
