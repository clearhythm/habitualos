...
(function () {
  ...
  const SIGNAL_ID = (script && script.getAttribute('data-signal-id')) || 'erik';
  const BASE_URL = script ? new URL(script.src).origin : 'https://signal.habitualos.com';
  const mode = script && script.getAttribute('data-signal-mode');
  const TESTING_MODE = mode === 'testing' || mode === 'coming-soon';
...
<div class="se-modal" id="se-modal">
  ...
  function injectComingSoon() {
    ...
    var email = ownerConfig && ownerConfig.contactLinks && ownerConfig.contactLinks.email;
    var ctaHtml = email
      ? '<a href="mailto:' + email + '" style="display:inline-block;background:#7c3aed;color:#fff;padding:0.65rem 1.5rem;border-radius:6px;text-decoration:none;font-weight:500;">Say Hello</a>'
      : '';
  ...
  if (TESTING_MODE) {
    domReady(function () {
      loadConfig().then(function (cfg) { ownerConfig = cfg; });
    });
  } else {
    domReady(init);
  }
